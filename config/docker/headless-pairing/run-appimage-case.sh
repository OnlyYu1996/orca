...
appimage=${SBBGT_TEST_APPIMAGE:-/artifacts/squashfs-root/AppRun}
timeout_seconds=${SBBGT_STARTUP_TIMEOUT_SECONDS:-12}
pairing_address=${SBBGT_PAIRING_ADDRESS:-127.0.0.1}
port=${SBBGT_SERVE_PORT:-0}
...
app_args=("${launcher[@]}")
if [[ ${SBBGT_TEST_NO_SANDBOX:-1} == 1 ]]; then
  app_args+=(--no-sandbox)
fi
app_args+=(serve --port "$port" --pairing-address "$pairing_address")
if [[ ${SBBGT_READY_JSON:-0} == 1 ]]; then
  app_args+=(--json)
fi
if [[ ${SBBGT_NO_PAIRING:-0} == 1 ]]; then
  app_args+=(--no-pairing)
fi
...
if [[ $is_appimage == 0 ]]; then
  export APPDIR=${SBBGT_TEST_APPDIR:-"$(dirname "$appimage")"}
fi

if [[ ${SBBGT_KEEP_RUNNING:-0} == 1 ]]; then
  exec "${command[@]}"
fi

...
async function startAndWait({
  ...
  const args = [
    ...
    '-e',
    'SBBGT_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `SBBGT_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `SBBGT_PAIRING_ADDRESS=${address}`,
    '-e',
    `SBBGT_SERVE_PORT=${port}`,
    '-e',
    `SBBGT_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'SBBGT_NO_PAIRING=1'] : []),
    '-v',
...
function hasCompleteReadyContract(stdout) {
  if (
    stdout.includes('赛博包工头 server ready\n') &&
...
function validateReady(logs, mode, expectedHost, options = {}) {
  if (mode === 'human') {
    assert(
      (logs.match(/^赛博包工头 server ready$/gm) ?? []).length === 1,
    ...
    assert(logs.includes('Pairing URL: sbbgt://pair?code='), 'human pairing URL is missing')
    return
  }

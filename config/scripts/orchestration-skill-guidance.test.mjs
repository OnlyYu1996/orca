...
describe('orchestration skill guidance', () => {
  it('requires 赛博包工头 runtime state before claiming a worker was orchestrated', () => {
    const skill = readSkill()
    const toolBoundary = getSection(skill, 'Tool Boundary')

    expect(toolBoundary).toContain(
      'must create 赛博包工头 runtime state with `sbbgt orchestration task-create` and `sbbgt orchestration dispatch --inject`'
    )
    expect(toolBoundary).toContain('or `sbbgt orchestration run`')
    expect(toolBoundary).toContain(
      'Do not substitute non-赛博包工头 subagent tools, generic agent-spawn APIs, or chat-only parallel worker features'
    )
    expect(toolBoundary).toContain('do not create 赛博包工头 task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('sbbgt orchestration task-list --json')
    expect(toolBoundary).toContain('sbbgt orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(skill).toContain(
      '`--no-parent` only controls 赛博包工头 lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
      'never base it on the current feature branch unless the user explicitly asks'
    )
    expect(skill).toContain(
      'sbbgt worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired 赛博包工头 lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('clarifies sidebar lineage for same-worktree orchestrated workers', () => {
    ...
    expect(workerTerminals).toContain(
      'A same-worktree worker created with `sbbgt terminal create --worktree active` may appear as a peer terminal/agent'
    )
    expect(workerTerminals).toContain(
      'even though it is a child dispatch in 赛博包工头 orchestration state'
    )
    expect(workerTerminals).toContain(
    ...
    expect(workerTerminals).toContain(
      'For supervised new-worktree workers, decide the desired 赛博包工头 lineage before creation'
    )
    expect(workerTerminals).toContain(
  ...
  it('keeps worker_done post-completion guidance idle instead of polling', () => {
    ...
    expect(agentGuidance).toContain('Do not poll or keep calling `sbbgt orchestration check`')
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('sbbgt terminal list')

...
const RESTRICTED_ENV_KEYS = [
  ...
  'SBBGT_CODEX_HOME',
  'ORCA_CODEX_HOME',
  'SBBGT_CODEX_SYSTEM_DEFAULT_REAL_HOME',
  'ORCA_CODEX_SYSTEM_DEFAULT_REAL_HOME',
  'SBBGT_E2E_HOME_DIR',
  'ORCA_E2E_HOME_DIR',
  'SBBGT_E2E_USER_DATA_DIR',
  'ORCA_E2E_USER_DATA_DIR',
  'SBBGT_USER_DATA_PATH',
  'ORCA_USER_DATA_PATH',
  'ZDOTDIR',
  'SBBGT_ORIG_ZDOTDIR',
  'ORCA_ORIG_ZDOTDIR',
...
export function createValidationEnv(inheritedEnv, layout, options = {}) {
  ...
  return {
    ...
    SBBGT_E2E_HOME_DIR: layout.homeDir,
    SBBGT_E2E_USER_DATA_DIR: layout.userDataDir,
    SBBGT_USER_DATA_PATH: layout.userDataDir,
    ...
    SBBGT_CODEX_SYSTEM_DEFAULT_REAL_HOME: options.systemDefaultRealHome === 'off' ? '0' : '1'
  }
}
...
export async function createValidationLayout(options = {}) {
  ...
  const envTempParent =
    process.env.SBBGT_CODEX_VALIDATION_TEMP_PARENT?.trim() ??
    process.env.ORCA_CODEX_VALIDATION_TEMP_PARENT?.trim()
  ...
  if (samePath(tempParentReal, primaryHomeReal) || isWithin(tempParentReal, primaryHomeReal)) {
    throw new Error(
      `Refusing to place the disposable validation root inside the primary home (${primaryHome}). ` +
        'Pass --temp-parent <dir> or set SBBGT_CODEX_VALIDATION_TEMP_PARENT to a directory outside it.'
    )
  }
...
function validationCliCommand() {
  if (process.env.SBBGT_VALIDATION_CLI) {
    return process.env.SBBGT_VALIDATION_CLI
  }
  if (process.env.ORCA_VALIDATION_CLI) {
    return process.env.ORCA_VALIDATION_CLI
  }
  if (process.env.SBBGT_CLI_COMMAND ?? process.env.ORCA_CLI_COMMAND) {
    return process.env.SBBGT_CLI_COMMAND ?? process.env.ORCA_CLI_COMMAND
  }
  return 'sbbgt'
}

async function probeTerminalEnvironment(terminalHandle, launchEnv) {
  const marker = `__SBBGT_CODEX_VALIDATION_${randomUUID()}__`
  const command = [
    'node -e',
    `"console.log('${marker}:' + JSON.stringify({home: require('node:os').homedir(), codexHome: process.env.CODEX_HOME || null, sbbgtCodexHome: process.env.SBBGT_CODEX_HOME || process.env.ORCA_CODEX_HOME || null}))"`
  ].join(' ')

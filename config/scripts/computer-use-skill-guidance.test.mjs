...
describe('computer-use skill guidance', () => {
  it('keeps web-app targeting on the computer-use surface', () => {
    ...
    expect(skill).toContain('Use this skill for desktop UI through `sbbgt computer`')
    expect(skill).toContain('operate the desktop browser app/window that contains the page')
    expect(skill).not.toContain('sbbgt goto')
    expect(skill).not.toContain('sbbgt snapshot')
    expect(skill).not.toContain('sbbgt click')
    expect(skill).not.toContain('sbbgt fill')

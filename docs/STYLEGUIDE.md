...
- **Default focus:** dialogs, popovers, and command surfaces should focus the field or primary action the user is most likely to use. A focused overlay with exactly one primary text input must focus that input as soon as it opens; close buttons and secondary actions must not win initial focus. Inline reusable components must not opt into autofocus globally — the dialog or popover that owns the interaction enables it explicitly. When the overlay closes, return focus to its trigger. Prefer native `autoFocus` or Radix `onOpenAutoFocus`; don't use timers to steal focus. If Enter submits, focus must land where Enter triggers the intended primary action. Esc should back out without adding visual noise to Cancel/Dismiss.

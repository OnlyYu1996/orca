...
const JOURNAL_STORAGE_KEY = 'sbbgt:mobile-relay:pairing-journal:v1'
const JOURNAL_SECRET_KEY = 'sbbgt.mobile-relay.pairing-journal.v1'
const LEGACY_JOURNAL_STORAGE_KEY = 'orca:mobile-relay:pairing-journal:v1'
const LEGACY_JOURNAL_SECRET_KEY = 'orca.mobile-relay.pairing-journal.v1'
...
export async function saveMobileRelayPairingJournal(
  ...
  const mutation = journalMutation.then(async () => {
    const existingRaw =
      ...
      (await AsyncStorage.getItem(LEGACY_JOURNAL_STORAGE_KEY))
...
export async function loadMobileRelayPairingJournal(): Promise<MobileRelayPairingJournal | null> {
  ...
  const currentMetadata = await AsyncStorage.getItem(JOURNAL_STORAGE_KEY)
  const rawMetadata = currentMetadata ?? (await AsyncStorage.getItem(LEGACY_JOURNAL_STORAGE_KEY))
  if (rawMetadata === null) {
    await SecureStore.deleteItemAsync(JOURNAL_SECRET_KEY, KEYCHAIN_OPTIONS).catch(() => {})
    await SecureStore.deleteItemAsync(LEGACY_JOURNAL_SECRET_KEY, KEYCHAIN_OPTIONS).catch(() => {})
    return null
  }
  ...
  const currentSecrets = await SecureStore.getItemAsync(JOURNAL_SECRET_KEY, KEYCHAIN_OPTIONS)
  const rawSecrets =
    currentSecrets ?? (await SecureStore.getItemAsync(LEGACY_JOURNAL_SECRET_KEY, KEYCHAIN_OPTIONS))
  if (rawSecrets === null) {
    await AsyncStorage.removeItem(JOURNAL_STORAGE_KEY)
    await AsyncStorage.removeItem(LEGACY_JOURNAL_STORAGE_KEY)
    return null
  }
  ...
  if (currentMetadata === null || currentSecrets === null) {
    // 新旧记录全部校验通过后再复制；旧键保留给回滚版本继续读取。
    await AsyncStorage.setItem(JOURNAL_STORAGE_KEY, rawMetadata)
    await SecureStore.setItemAsync(JOURNAL_SECRET_KEY, rawSecrets, KEYCHAIN_OPTIONS)
  }
...
export async function clearMobileRelayPairingJournal(journalId: string): Promise<void> {
  const mutation = journalMutation.then(async () => {
    ...
    await SecureStore.deleteItemAsync(LEGACY_JOURNAL_SECRET_KEY, KEYCHAIN_OPTIONS)
  })
  journalMutation = mutation.catch(() => {})
  return mutation
}
...
function requireNativeSecretStore(): void {
  if (Platform.OS === 'web') {
    throw new Error('赛博包工头 Relay 配对需要原生安全存储')
  }
}

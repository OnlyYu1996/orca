...
const OVERLAY_STORAGE_KEY = 'sbbgt:mobile-relay:host-overlays:v2'
const LEGACY_OVERLAY_STORAGE_KEY = 'orca:mobile-relay:host-overlays:v2'
let overlayMutation: Promise<void> = Promise.resolve()

async function readOverlayStorage(): Promise<string | null> {
  return (
    (await AsyncStorage.getItem(OVERLAY_STORAGE_KEY)) ??
    (await AsyncStorage.getItem(LEGACY_OVERLAY_STORAGE_KEY))
  )
}
...
async function readOverlaysForMutation(): Promise<MobileRelayHostOverlay[]> {
  const overlays = parseOverlays(await readOverlayStorage())
  if (!overlays) {

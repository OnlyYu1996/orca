...
vi.mock('./i18n/mobile-locale-context', async () => {
  const { enMessages } = await import('./i18n/mobile-locales')
  const translate = (key: keyof typeof enMessages, parameters?: Record<string, string | number>) =>
    enMessages[key].replace(/{{(\w+)}}/g, (placeholder, name) =>
      parameters?.[name] == null ? placeholder : String(parameters[name])
    )
  return {
    useMobileLocale: () => ({
      locale: 'en',

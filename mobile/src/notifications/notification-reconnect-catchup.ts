...
const LAST_SEQ_STORAGE_KEY_PREFIX = 'sbbgt:mobileNotificationsLastSeq:'
const LEGACY_LAST_SEQ_STORAGE_KEY_PREFIX = 'orca:mobileNotificationsLastSeq:'

function lastSeqStorageKey(hostId: string): string {
  return LAST_SEQ_STORAGE_KEY_PREFIX + encodeURIComponent(hostId)
}

function legacyLastSeqStorageKey(hostId: string): string {
  return LEGACY_LAST_SEQ_STORAGE_KEY_PREFIX + encodeURIComponent(hostId)
}

export async function loadLastSeenSeq(hostId: string): Promise<number> {
  try {
    const raw =
      (await AsyncStorage.getItem(lastSeqStorageKey(hostId))) ??
      (await AsyncStorage.getItem(legacyLastSeqStorageKey(hostId)))

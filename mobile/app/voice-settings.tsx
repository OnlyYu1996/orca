...
export default function VoiceSettingsScreen(): React.JSX.Element {
  ...
  const dictationModes = [
    { value: 'toggle', label: t('voice.toggle') },
    { value: 'hold', label: t('voice.hold') }
  ] as const
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t('voice.loadFailed'))
    }
  }, [client, t])
  ...
  const handleToggleEnabled = useCallback(
    async (enabled: boolean) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t('voice.updateFailed'))
        void refresh()
      }
    },
    [client, refresh, t]
  ...
  const handleUseModel = useCallback(
    async (model: MobileSpeechModel) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t('voice.selectFailed'))
      } finally {
        setBusyAction(null)
      }
    },
  ...
  const handleDownload = useCallback(
    async (model: MobileSpeechModel) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t('voice.downloadFailed'))
      } finally {
        setBusyAction(null)
      }
    },
  ...
  const handleDelete = useCallback(
    async (model: MobileSpeechModel) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : t('voice.deleteFailed'))
      } finally {
        setBusyAction(null)
      }
    },
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + spacing.sm }]}>
      ...
      {!client ? (
        ...
        <View style={[styles.section, styles.sectionTopGap]}>
          <Text style={styles.errorText}>{error ?? t('voice.loadFailed')}</Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          <Text style={styles.groupHeading}>{t('voice.dictationHeading')}</Text>
          <View style={[styles.section, styles.sectionTopGap]}>
            <View style={styles.row}>
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>{t('voice.enableDictation')}</Text>
                <Text style={styles.rowSublabel}>{t('voice.enableDescription')}</Text>
              </View>
              <Switch
                value={enabled}
            ...
            >
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>{t('voice.mode')}</Text>
                <Text style={styles.rowSublabel}>{t('voice.modeDescription')}</Text>
              </View>
          ...
          <View style={[styles.section, styles.sectionTopGap]}>
            ...
            >
              <View style={styles.rowContent}>
                <Text style={styles.rowLabel}>{t('voice.speechModel')}</Text>
      ...
      <BottomDrawer visible={modelDrawerOpen} onClose={() => setModelDrawerOpen(false)}>
        <Text style={styles.drawerTitle}>{t('voice.speechModel')}</Text>
        {setup ? (
          <VoiceModelList
            setup={setup}
            disabled={false}

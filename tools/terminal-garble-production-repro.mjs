...
const DEFAULT_EXECUTABLE = '/Applications/赛博包工头.app/Contents/MacOS/赛博包工头'
const DEFAULT_PROFILE = path.join(os.homedir(), 'Library', 'Application Support', 'sbbgt')
const URL = 'https://example.com/sbbgt-terminal-garble-repro'
...
const runRoot = mkdtempSync(path.join(os.tmpdir(), 'sbbgt-terminal-garble-'))
const userDataDir = path.join(runRoot, 'user-data')
const evidenceDir = path.join(runRoot, 'evidence')
mkdirSync(userDataDir, { recursive: true })
mkdirSync(evidenceDir, { recursive: true })
function sanitizeProfile() {
  const sourceDataPath = path.join(sourceProfile, 'sbbgt-data.json')
  ...
  writeFileSync(path.join(userDataDir, 'sbbgt-data.json'), serialized)

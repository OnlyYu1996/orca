...
**Windows only.** Proves that a **crash of 赛博包工头's main process** does not orphan
...
On Windows, when 赛博包工头's main/renderer process crashed, open terminal PTYs were
...
1. **Launch** the installed `sbbgt.exe` under an isolated `userData` dir,
   seeded with a fresh profile (onboarding dismissed
...
2. **Stamp the interactive shell** — typing DIRECTLY into it (not a nested
   `powershell`), set a per-shell env sentinel `SBBGT_CRASH_SENTINEL=<canary>` and
...
6. **Relaunch** (same `userData`, no reseed) and assert the daemon PID is
   **unchanged** (the new main **adopts** the surviving daemon instead of forking
   ...
   `SBBGT_CRASH_SENTINEL` and the shell's `$PID`, which a freshly re-spawned shell
   would not carry.
...
8. **Teardown** — close the relaunched app, then kill this run's scoped daemon
   **tree** (re-discovered fresh via `findDaemonProcesses(userData)`, which the
   ...
   process), never installs/uninstalls, and never touches any other 赛博包工头 on the box.
...
node tools/win-crash-survival-e2e/run.mjs --expect survival --exe-path "C:\Users\<you>\AppData\Local\Programs\赛博包工头\sbbgt.exe"
```
...
- The crash kills **only** the real Electron main of the instance this harness
  ...
  name or a scanned pid, so a developer's live 赛博包工头 (a different `userData`, out of
  scope) is untouched.

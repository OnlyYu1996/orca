...
//
// GitHub #7742: on Windows, when 赛博包工头's main/renderer process crashed, open
...
const SENTINEL_ENV = 'SBBGT_CRASH_SENTINEL'
...
async function main() {
  ...
  // 先校验平台，避免非 Windows 环境被可执行文件定位错误干扰。
  ...
  const canary = `SBBGT-CRASH-SENTINEL-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `sbbgt-win-crash-e2e-${runId}-`))
  ...
  const diagDir = process.env.SBBGT_E2E_DIAG_DIR || path.join(runDir, 'diag')
  let passed = false
  try {
 ...
 * daemons on the machine (including the developer's live 赛博包工头) are ignored.
 ...
 * touches any other 赛博包工头 on the box (a live user instance uses a different

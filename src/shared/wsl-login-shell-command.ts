...
export function buildWslInteractiveLoginShellCommand(): string {
  return [
    ...
    'if [ -n "${SBBGT_USER_DATA_PATH:-${ORCA_USER_DATA_PATH:-}}" ]; then',
    '  _orca_shell_ready_root="${SBBGT_USER_DATA_PATH:-${ORCA_USER_DATA_PATH}}"',
    '  _orca_shell_ready_root="${_orca_shell_ready_root%/}/shell-ready"',
    'fi',
    '_orca_wsl_shell_name=$(basename "$_orca_wsl_shell" | tr "[:upper:]" "[:lower:]")',
    'case "$_orca_wsl_shell_name" in',
    '  bash)',

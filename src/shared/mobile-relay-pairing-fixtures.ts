...
import { PRODUCT_URL_SCHEME } from './product-identity'
...
export function createMobileRelayPairingFixtures(now: number): PairingFixture[] {
  ...
  const relay = {
    v: 1 as const,
    directorUrl: 'https://relay.sbbgt.example',
    cellUrl: 'https://relay-c1.sbbgt.example',
    assignmentEpoch: 7,
  ...
  return [
    ...
    },
    {
      ...
      payload: { ...directOffer, relay: { ...relay, directorUrl: 'https://relay.sbbgt.example/' } },
      expected: null
    },
    {
      name: 'non-HTTPS cell origin is invalid',
      payload: { ...directOffer, relay: { ...relay, cellUrl: 'http://relay-c1.sbbgt.example' } },
      expected: null
    },
    {
...
export function encodePairingFixturePayload(payload: unknown): string {
  ...
  return `${PRODUCT_URL_SCHEME}://pair?code=${code}`
}

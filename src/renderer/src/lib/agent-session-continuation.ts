...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `赛博包工头 pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior 赛博包工头 session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest 赛博包工头 status hints:', ...statusHints] : []),
    '',

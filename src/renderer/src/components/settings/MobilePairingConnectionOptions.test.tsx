...
describe('MobilePairingConnectionOptions', () => {
  ...
  it('offers local-only pairing while Relay requires sign-in', async () => {
    ...
    expect(screen.getByRole('switch', { name: /赛博包工头 Relay/i })).toBeDisabled()
    expect(screen.getByRole('switch', { name: /赛博包工头 Relay/i })).not.toBeChecked()
  ...
  it('selects either automatic fallback or local-only pairing when signed in', async () => {
    ...
    expect(screen.getByRole('switch', { name: /赛博包工头 Relay/i })).toBeChecked()
    await user.click(screen.getByRole('switch', { name: /赛博包工头 Relay/i }))
  ...
  it('keeps the compact onboarding choices structurally stable across modes', async () => {
    ...
    expect(screen.getByRole('switch', { name: /赛博包工头 Relay/i })).toBeChecked()
    expect(screen.getByText(/优先使用直连/i)).toBeVisible()
    expect(screen.queryByText('Ready')).toBeNull()

    rerender(<MobilePairingConnectionOptions {...props} value="local-only" />)
    expect(screen.getByRole('switch', { name: /赛博包工头 Relay/i })).not.toBeChecked()
    expect(screen.getByText(/优先使用直连/i)).toBeVisible()
    expect(screen.queryByText(/不通过赛博包工头 Relay/i)).toBeNull()
  })
})

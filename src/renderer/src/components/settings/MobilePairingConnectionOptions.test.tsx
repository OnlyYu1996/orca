...
describe('MobilePairingConnectionOptions', () => {
  ...
  it('shows a compact Sign in row when 赛博包工头 Relay is selected and signed out', async () => {
    ...
    expect(screen.getByText('Sign in to use 赛博包工头 Mobile Relay.')).toBeVisible()
  ...
  it('moves selection with the arrow keys as a radiogroup', async () => {
    ...
    screen.getByRole('radio', { name: /赛博包工头 Relay/i }).focus()
  ...
  it('selects a path from the compact list', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /赛博包工头 Relay/i }))
    expect(onChange).toHaveBeenCalledWith('automatic')
  })
  ...
  it('shows relay status when signed in on 赛博包工头 Relay', async () => {
    mocks.state = {
      orcaProfileAuthStatus: {
        activeProfileId: 'profile-1',
        configured: true,

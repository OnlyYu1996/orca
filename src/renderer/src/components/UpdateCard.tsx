...
import { PRODUCT_RELEASES_URL, getProductReleaseTagUrl } from '../../../shared/product-links'
import {
...
function releaseUrlForVersion(version: string | null): string {
  // Why: 未缓存版本时使用普通发布列表，避免 GitHub 发布 API 异常时 /latest 也失效。
  return version ? getProductReleaseTagUrl(version) : PRODUCT_RELEASES_URL
}
        ...
        : isSignatureMismatchError
          ? {
              ...
              summary: translate(
                ...
                "The installer's publisher doesn't match 赛博包工头, so we stopped the update. Don't install this download; check the 赛博包工头 releases for a corrected version."
              ),
          ...
          : isSignatureCheckBlockedError
            ? {
                ...
                summary: translate(
                  ...
                  "The signature check couldn't run — usually because antivirus software blocked it. Retry the download, or get the installer from the 赛博包工头 releases."
                ),
                detail: status.message,
                releaseUrl: releaseUrlForVersion(cachedVersion),
                primaryAction: {

...
describe('orca skills CLI', () => {
  ...
  it('shows leaf, group, and root help for skills', async () => {
    ...
    expect(String(logSpy.mock.calls[0]?.[0])).toContain(
      '用法：sbbgt skills get <topic> [--full] [--json]'
    )
    expect(String(logSpy.mock.calls[1]?.[0])).toContain(
      '列出赛博包工头 CLI 内置的版本匹配 Skill 指南'
    )
    expect(String(logSpy.mock.calls[1]?.[0])).toContain('以 Markdown 输出版本匹配的 Skill 指南')
    expect(String(logSpy.mock.calls[2]?.[0])).toContain('Skill 指南：\n  skills list')
    expect(runtimeClientConstructorMock).not.toHaveBeenCalled()
  })

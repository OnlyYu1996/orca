...
export async function resumeInterruptedServeUpdate(
  ...
  if (!installed) {
    await recordServeUpdateHandoffFailure(
      args.handoffPath,
      args.handoff,
      `Timed out waiting for 赛博包工头 ${args.handoff.targetVersion} to be installed.`
    )
  }
  ...
  while (true) {
    ...
    if (
      ...
      throw new RuntimeClientError(
        ...
        `赛博包工头 serve exited via ${result.signal}`
      )
    }
    ...
    if (!installed) {
      await recordServeUpdateHandoffFailure(
        args.handoffPath!,
        handoff,
        `Timed out waiting for 赛博包工头 ${handoff.targetVersion} to be installed.`
      )

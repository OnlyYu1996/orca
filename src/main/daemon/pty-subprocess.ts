...
import {
  LEGACY_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY_ENV,
...
function composeGuardedDaemonGitConfigEnv(
  ...
  const policy =
    explicitEnv?.[TERMINAL_GIT_CREDENTIAL_GUARD_POLICY_ENV] ??
    explicitEnv?.[LEGACY_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY_ENV]
  delete env[TERMINAL_GIT_CREDENTIAL_GUARD_POLICY_ENV]
  delete env[LEGACY_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY_ENV]
...
function deleteRequestedDaemonEnvKeys(
  ...
  const managedCodexHome = env.SBBGT_CODEX_HOME ?? env.ORCA_CODEX_HOME
  const deleteOrcaOwnedCodexHome =
    (keys?.includes('SBBGT_CODEX_HOME') === true || keys?.includes('ORCA_CODEX_HOME') === true) &&
    managedCodexHome !== undefined &&
...
export function createPtySubprocess(opts: PtySubprocessOptions): SubprocessHandle {
  ...
  if (process.platform === 'win32') {
    ...
    if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
      ...
      // Why: WSL Codex Home 是 Linux 路径，宿主 Shell 启动时必须同时清除主标记和兼容标记。
      delete env.CODEX_HOME
      delete env.SBBGT_CODEX_HOME
      delete env.ORCA_CODEX_HOME
    }

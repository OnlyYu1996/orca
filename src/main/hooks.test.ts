  ...
  it('runs WSL hooks through wsl.exe and translates env paths to Linux', async () => {
    ...
    execFileMock.mockImplementation((_file, _args, options, callback) => {
      ...
      expect(options).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            SBBGT_ROOT_PATH: '/mnt/c/Users/jinwo/git/orca',
            SBBGT_WORKTREE_PATH: '/mnt/c/Users/jinwo/git/orca-feature',
            SBBGT_WORKSPACE_NAME: 'orca-feature',
        ...
        expect.objectContaining({
          env: expect.objectContaining({
            ...
            WSLENV: expect.stringContaining(
              'SBBGT_ROOT_PATH/u:SBBGT_WORKTREE_PATH/u:ORCA_ROOT_PATH/u:ORCA_WORKTREE_PATH/u:CONDUCTOR_ROOT_PATH/u:GHOSTX_ROOT_PATH/u:SBBGT_WORKSPACE_NAME/u:ORCA_WORKSPACE_NAME/u'
            )
          })
        })
      )
...
describe('createSetupRunnerScript', () => {
  ...
  it('marks setup-runner terminals for the always-on credential guard', async () => {
    ...
    expect(setup.envVars).toMatchObject({
      ...
      SBBGT_INTERNAL_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY: 'guard'
    })
  })
})

 ...
 */
const CODEX_REAL_HOME_ENV_FLAG = 'SBBGT_CODEX_SYSTEM_DEFAULT_REAL_HOME'
const LEGACY_CODEX_REAL_HOME_ENV_FLAG = 'ORCA_CODEX_SYSTEM_DEFAULT_REAL_HOME'
...
function readCodexRealHomeEnvOverride(): boolean | null {
  const raw = process.env[CODEX_REAL_HOME_ENV_FLAG] ?? process.env[LEGACY_CODEX_REAL_HOME_ENV_FLAG]
  if (raw === undefined) {
    return null
  }

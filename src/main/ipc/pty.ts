...
const CODEX_HOME_ENV_KEYS = ['CODEX_HOME', 'SBBGT_CODEX_HOME', 'ORCA_CODEX_HOME'] as const
...
function getLocalOrcaCodexHomeEnvKeysToDelete(env: Record<string, string>): string[] {
  const inheritedOrcaOverride =
    env.SBBGT_CODEX_HOME ??
    process.env.SBBGT_CODEX_HOME ??
    env.ORCA_CODEX_HOME ??
    process.env.ORCA_CODEX_HOME
  const inheritedCodexHome = env.CODEX_HOME ?? process.env.CODEX_HOME
  const keysToDelete = ['SBBGT_CODEX_HOME', 'ORCA_CODEX_HOME']
...
function removeCodexHomeDeletionRequests(keys: string[] | undefined): string[] | undefined {
  ...
  const filtered = keys?.filter(
    (key) => key !== 'CODEX_HOME' && key !== 'SBBGT_CODEX_HOME' && key !== 'ORCA_CODEX_HOME'
  )
...
export function buildPtyHostEnv(
  ...
  if (opts.skipCodexHomeEnv) {
    ...
    delete baseEnv.SBBGT_CODEX_HOME
    ...
    baseEnv.SBBGT_CODEX_HOME = opts.selectedCodexHomePath
  ...
  if (opts.isWsl) {
    ...
    // Why: WSL 需显式导出托管 CLI 命令，以避免误用宿主系统中的同名程序。
  ...
  if (!opts.isPackaged) {
    ...
    // Why: 历史兼容启动器只注入产品 PTY，避免遮蔽 GNOME 的屏幕阅读器（上游 #7904）。
...
export function registerHeadlessPtyRuntime(
  ...
  // Why: 无头 `sbbgt serve` 没有渲染窗口，但远程客户端仍需要同一组 PTY 处理器。

  ...
  if (process.platform === 'darwin') {
    if (attentionActive) {
      try {
        ...
        tray.setToolTip(
          translateMain('tray.activityWaiting', `${PRODUCT_DISPLAY_NAME} - 活动等待中`)
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    ...(process.platform === 'darwin'
      ? ([
          {
            label: translateMain('menu.settings', '设置'),
            click: safeMenuAction(() => opts.onOpenSettings())
          },
          {
            label: translateMain('menu.checkForUpdates', '检查更新...'),
            click: safeMenuAction(() => opts.onCheckForUpdates())
          },
          { type: 'separator' }
        ] as Electron.MenuItemConstructorOptions[])
      : []),
    { label: translateMain('tray.quit', '退出'), click: safeMenuAction(() => opts.onQuit()) }
  ])

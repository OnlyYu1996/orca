...
describe('getManagedStatusLineScript (win32 local)', () => {
  ...
  it('posts the buffered payload file and deletes it afterwards', () => {
    ...
    expect(script).toContain('set "SBBGT_STATUSLINE_PANE_ID=%ORCA_PANE_KEY:~-36%"')
    expect(script).toContain('set "SBBGT_STATUSLINE_PANE_ID=%SBBGT_STATUSLINE_PANE_ID::=_%"')
    expect(script).toContain(
      'set "SBBGT_STATUSLINE_PAYLOAD_FILE=%TEMP%\\sbbgt-claude-statusline-%SBBGT_STATUSLINE_PANE_ID%.tmp"'
    )
    expect(script).toContain('--data-urlencode "payload@%SBBGT_STATUSLINE_PAYLOAD_FILE%"')
    expect(script).not.toContain('payload@-')
    const curlIndex = script.indexOf('curl.exe')
    const delIndex = script.indexOf('del "%SBBGT_STATUSLINE_PAYLOAD_FILE%"')
  ...
  it('never posts a literal %CLAUDE_CONFIG_DIR% token when the var is unset', () => {
    ...
    expect(script).toContain('set "SBBGT_STATUSLINE_CONFIG_DIR_FIELD=configDir="')
    expect(script).toContain(
      'if defined CLAUDE_CONFIG_DIR set "SBBGT_STATUSLINE_CONFIG_DIR_FIELD=configDir=%CLAUDE_CONFIG_DIR%"'
    )
    expect(script).toContain('--data-urlencode "%SBBGT_STATUSLINE_CONFIG_DIR_FIELD%"')
  ...
  it('throttles with an all-builtin seconds-of-day stamp that fails open to posting', () => {
    ...
    const throttleIndex = script.indexOf(
      `if %SBBGT_STATUSLINE_ELAPSED% GEQ 0 if %SBBGT_STATUSLINE_ELAPSED% LSS ${CLAUDE_STATUSLINE_MIN_POST_INTERVAL_SECONDS} goto :orca_statusline_cleanup`
    )
    ...
    expect(script).toContain('if not defined SBBGT_STATUSLINE_ELAPSED goto :orca_statusline_probe')
    expect(script).toContain(
      'for /f "delims=0123456789" %%d in ("%SBBGT_STATUSLINE_LAST%") do set "SBBGT_STATUSLINE_LAST="'
    )
    expect(script).toContain(
    ...
    expect(script).toContain('set "SBBGT_STATUSLINE_TIME=%TIME: =0%"')
  })
})
...
describe('statusline curl throttle (posix)', () => {
  it('checks the per-pane stamp after the env guards and before curl', () => {
    ...
    const stampIndex = script.indexOf('sbbgt-claude-statusline-last-${orca_statusline_pane_id}')
...
describe.skipIf(process.platform === 'win32')('statusline curl throttle (posix behavioral)', () => {
  ...
  function stampPathFor(dir: string, leafId = LEAF_ID): string {
    return join(dir, `sbbgt-claude-statusline-last-${leafId}`)
  }

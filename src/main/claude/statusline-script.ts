...
export function getManagedStatusLineScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      'set "SBBGT_STATUSLINE_PANE_ID=%ORCA_PANE_KEY:~-36%"',
      'set "SBBGT_STATUSLINE_PANE_ID=%SBBGT_STATUSLINE_PANE_ID::=_%"',
      ...
      'set "SBBGT_STATUSLINE_PAYLOAD_FILE=%TEMP%\\sbbgt-claude-statusline-%SBBGT_STATUSLINE_PANE_ID%.tmp"',
      `${WINDOWS_HOOK_STDIN_READER} >"%SBBGT_STATUSLINE_PAYLOAD_FILE%" 2>nul`,
      // Why: an all-builtin seconds-of-day throttle avoids spawning findstr+curl on every streaming tick.
      'set "SBBGT_STATUSLINE_STAMP_FILE=%TEMP%\\sbbgt-claude-statusline-last-%SBBGT_STATUSLINE_PANE_ID%.tmp"',
      'set "SBBGT_STATUSLINE_NOW="',
      'set "SBBGT_STATUSLINE_TIME=%TIME: =0%"',
      'for /f "tokens=1-3 delims=:.," %%a in ("%SBBGT_STATUSLINE_TIME%") do set /a "SBBGT_STATUSLINE_NOW=(1%%a %% 100)*3600+(1%%b %% 100)*60+(1%%c %% 100)" 2>nul',
      'set "SBBGT_STATUSLINE_LAST="',
      'set "SBBGT_STATUSLINE_ELAPSED="',
      'if exist "%SBBGT_STATUSLINE_STAMP_FILE%" set /p SBBGT_STATUSLINE_LAST=<"%SBBGT_STATUSLINE_STAMP_FILE%"',
      'if defined SBBGT_STATUSLINE_LAST for /f "delims=0123456789" %%d in ("%SBBGT_STATUSLINE_LAST%") do set "SBBGT_STATUSLINE_LAST="',
      'if defined SBBGT_STATUSLINE_NOW if defined SBBGT_STATUSLINE_LAST set /a "SBBGT_STATUSLINE_ELAPSED=SBBGT_STATUSLINE_NOW-SBBGT_STATUSLINE_LAST" 2>nul',
      `if not defined SBBGT_STATUSLINE_ELAPSED goto :${STATUSLINE_PROBE_LABEL}`,
      `if %SBBGT_STATUSLINE_ELAPSED% GEQ 0 if %SBBGT_STATUSLINE_ELAPSED% LSS ${CLAUDE_STATUSLINE_MIN_POST_INTERVAL_SECONDS} goto :${STATUSLINE_CLEANUP_LABEL}`,
      ...
      '"%SystemRoot%\\System32\\findstr.exe" /c:\\"rate_limits\\" "%SBBGT_STATUSLINE_PAYLOAD_FILE%" >nul 2>nul',
      ...
      'if defined SBBGT_STATUSLINE_NOW (>"%SBBGT_STATUSLINE_STAMP_FILE%" echo %SBBGT_STATUSLINE_NOW%)',
      ...
      'set "SBBGT_STATUSLINE_CONFIG_DIR_FIELD=configDir="',
      'if defined CLAUDE_CONFIG_DIR set "SBBGT_STATUSLINE_CONFIG_DIR_FIELD=configDir=%CLAUDE_CONFIG_DIR%"',
      [
        ...
        '--data-urlencode "%SBBGT_STATUSLINE_CONFIG_DIR_FIELD%"',
        '--data-urlencode "env=%ORCA_AGENT_HOOK_ENV%"',
        '--data-urlencode "version=%ORCA_AGENT_HOOK_VERSION%"',
        '--data-urlencode "payload@%SBBGT_STATUSLINE_PAYLOAD_FILE%"',
        '>nul 2>&1'
      ].join(' '),
      `:${STATUSLINE_CLEANUP_LABEL}`,
      'del "%SBBGT_STATUSLINE_PAYLOAD_FILE%" >nul 2>nul',
      'exit /b 0',
      ...buildWindowsHookStdinDrainEpilogue(),
      ''
    ].join('\r\n')
  ...
  return [
    ...
    '    ;;',
    'esac',
    'orca_statusline_stamp="${TMPDIR:-/tmp}/sbbgt-claude-statusline-last-${orca_statusline_pane_id}"',

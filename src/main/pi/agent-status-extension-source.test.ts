...
// 所有权以 process.pid 为键，因此重载和子进程测试需要稳定且不同的身份。
const SELF_PID = 4242
...
describe('getPiAgentStatusExtensionSource', () => {
  ...
  it.each(['pi', 'omp'] as const)(
    ...
    (kind) => {
      // 继承主进程所有者 PID 后必须禁用整个扩展，避免后续 Hook 再次引入通知泄漏。
      ...
      expect(child.processEnv.SBBGT_PI_STATUS_OWNED).toBe(String(SELF_PID))
      expect(grandchild.processEnv.SBBGT_PI_STATUS_OWNED).toBe(String(SELF_PID))
  ...
  it('reports agent_end for a top-level run (including non-interactive) and claims the pane by pid', async () => {
    ...
    expect(harness.processEnv.SBBGT_PI_STATUS_OWNED).toBe(String(SELF_PID))
  })

  it('keeps reporting after the lead re-runs the extension factory on reload', async () => {
    // Pi 会在进程内重载扩展，主进程必须识别自己的 PID，不能把自身标记误判为子智能体。
  ...
  it('reads the legacy owner marker without writing it back', () => {
    const harness = createHarness({
      kind: 'pi',
      pid: SELF_PID,
    ...
    expect(harness.processEnv.SBBGT_PI_STATUS_OWNED).toBeUndefined()
    expect(harness.processEnv.ORCA_PI_STATUS_OWNED).toBe(String(SELF_PID - 1))
  })

  it('prefers the new owner marker over the legacy fallback', () => {
    const harness = createHarness({
      kind: 'pi',
      pid: SELF_PID,
      env: {
        SBBGT_PI_STATUS_OWNED: String(SELF_PID),
        ORCA_PI_STATUS_OWNED: String(SELF_PID - 1)
      }
    })

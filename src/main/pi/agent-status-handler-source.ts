...
export function getPiAgentStatusHandlerSourceLines(kind: PiAgentKind): string[] {
  ...
  return [
    ...
    '// 子智能体会继承主智能体的窗格环境；只有所有者进程可以注册状态 Hook。',
    '// 使用 PID 标识所有者，同时允许同一进程内的扩展重载继续上报。',
    'export default function (pi): void {',
    '  const ownerPid = process.env.SBBGT_PI_STATUS_OWNED ?? process.env.ORCA_PI_STATUS_OWNED',
    '  const selfPid = String(process.pid)',
    '  if (ownerPid && ownerPid !== selfPid) return',
    '  process.env.SBBGT_PI_STATUS_OWNED = selfPid',
    ...sessionStartHandler,

...
function deferHeadlessServeInstall(phase: 'download' | 'install', version: string): boolean {
  ...
  if (lastInstallDeferralVersion[phase] !== diagnosticVersion) {
    ...
    recordUpdaterLifecycle(
      ...
      {
        level: 'warn',
        message: 'Update install deferred while hosting sbbgt serve'
      }
    )
  }
  sendErrorStatus(
    'This sbbgt serve process was not started by an update-capable supervisor. Keep it running and update 赛博包工头 through its service manager.',
    true
  )
  return true
}
  ...
  try {
    await withUpdaterSpan({ stage: 'install' }, async (span) => {
      ...
      if (
        ...
        sendErrorStatus(
          'Could not prepare the supervised server restart. 赛博包工头 remains running.',
          true
        )
...
function scheduleAutomaticUpdateCheck(delayMs: number): void {
  ...
  autoUpdateCheckTimer = setTimeout(() => {
    // Why: 赛博包工头 runs for days, so keep the next background check scheduled in the main process rather than tying it to relaunches or renderer lifetime.

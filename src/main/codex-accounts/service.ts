...
export class CodexAccountService {
  ...
  private async doAddAccount(target?: CodexAccountAddTarget): Promise<CodexRateLimitAccountsState> {
    ...
    try {
      ...
      if (!identity.email) {
        throw new Error(
          'Codex login completed, but 赛博包工头 could not resolve the account email.'
        )
      }
  ...
  private async doReauthenticateAccount(accountId: string): Promise<CodexRateLimitAccountsState> {
    ...
    if (!identity.email) {
      throw new Error('Codex login completed, but 赛博包工头 could not resolve the account email.')
    }
  ...
  private createManagedHome(
    ...
    // Why: marker lets future cleanup prove the path belongs to 赛博包工头 before deleting anything.
  ...
  private syncCanonicalConfigIntoManagedHome(
    ...
    // Why: 赛博包工头 account switching is meant to swap Codex credentials and quota
    ...
    if (isCodexSystemDefaultRealHomeEnabled()) {
      ...
      // Why: source-home 赛博包工头 trust is foreign to each managed home's hooks.json.
  ...
  private assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `赛博包工头 cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. 赛博包工头 left your config unchanged.`
    )
  }
  ...
  private recreateExpectedHostManagedHomeForReauthentication(
    ...
    // Why: re-auth may recreate a lost empty home, but only at the exact 赛博包工头-owned path persisted for this account.
  ...
  private assertManagedHomePath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        ...
        throw new Error('Managed WSL Codex home is outside 赛博包工头 account storage.')
      }
      if (
      ...
      if (!existsSync(join(candidatePath, '.orca-managed-home'))) {
        throw new Error('Managed Codex home is missing 赛博包工头 ownership marker.')
      }
      if (
        expectedAccountId !== undefined &&

  ...
  void maybeAutoRenameBranchOnFirstWork(
    ...
    },
    {
      ...
      canRenameOrcaCreatedBranch: (worktreeId) => {
        ...
        // Why: a user branch could coincidentally match a creature name; only 赛博包工头-stamped worktrees are safe to auto-rename.
...
if (startupDiagnosticsEnabled) {
  logStartupDiagnostic('before-single-instance-lock', {
    ...
    e2eUserData: Boolean(process.env.SBBGT_E2E_USER_DATA_DIR ?? process.env.ORCA_E2E_USER_DATA_DIR)
  })
...
if (hasSingleInstanceLock) {
  // Why: couple to dev-parent only for electron-vite desktop runs; `sbbgt serve`'s parent (CLI shim/background shell) isn't the intended server lifetime.
  ...
  // Why: run after configureDevUserDataPath but before app.setName('赛博包工头') (whenReady), which changes the resolved path on case-sensitive filesystems.
  initDataPath()
  ...
  const startupServices = startFirstWindowStartupServices({
    ...
    startAgentHookServer: async () => {
      ...
      await agentHookServer.start({
        ...
        // Why: 钩子调用时加载端点文件，可让旧 PTY 在重启后连到当前进程；开发模式使用 sbbgt-dev 命名空间。
    ...
    onAgentHookServerError: (error) => {
      // Why: hook callbacks are sidebar enrichment only; 赛博包工头 must still boot if the loopback receiver fails.
...
async function prepareCodexSessionResumeForLaunch(args: {
  ...
  if (!sessionSource) {
    if (args.providerSession.transcriptPath) {
      throw new Error(
        '赛博包工头 could not verify the originating Codex session file, so automatic resume was stopped to avoid using a different account.'
      )
    }
    return null
  }
  ...
  await serveReadinessPublisher.publish(
    {
      ...
      // Why: WSL 协调屏障会失败开放，pending 可提醒客户端 PTY 启动仍可能与修复竞争。
...
app.whenReady().then(async () => {
  ...
  // Why: one-time per-host backfill makes historical 赛博包工头-managed Codex
  ...
  runtimeService.setCommitMessageAgentEnvironmentResolvers({
    // Why: Codex hooks/auth live in 赛博包工头's managed runtime home even for the default path, so every launch must resolve CODEX_HOME via runtime-home.
    prepareForCodexLaunch: prepareCodexRuntimeHomeForLaunch,
    prepareForClaudeLaunch: (target) => claudeRuntimeAuth!.prepareForClaudeLaunch(target)
  })
  ...
  // Why: externally started serve-sim processes must stay independent — only 赛博包工头-managed/attached helpers belong to a workspace.
  ...
  const isE2E = Boolean(process.env.SBBGT_E2E_USER_DATA_DIR ?? process.env.ORCA_E2E_USER_DATA_DIR)
  // Why: pin dev to 6769 so `pnpm dev` doesn't race packaged 赛博包工头 on 6768 and fall back to a random port, breaking deterministic mobile pairing/repro (STA-1511).
  ...
  runtimeRpc = new OrcaRuntimeRpcServer({
    ...
    ...(serveOptions?.wsPort !== undefined
      ? {
          ...
          // Why: 仅显式 `sbbgt serve --port` 优先固定端口；默认模式仍优先回退值以保持旧移动端配对稳定。
  ...
  if (serveOptions) {
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      try {
        ...
        console.log(
          `[serve] sbbgt CLI install: ${cliStatus.state}${cliStatus.commandPath ? ` (${cliStatus.commandPath})` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] sbbgt CLI install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
    // Why: 旧 Claude Team 启动器可能仍调用兼容命令，因此在用户 PATH 中保留最佳尝试的转发器。

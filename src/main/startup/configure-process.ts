...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  if (e2eConfig.userDataDir) {
    ...
    const e2eHomeDir =
      process.env.SBBGT_E2E_HOME_DIR ??
      process.env.ORCA_E2E_HOME_DIR ??
      join(e2eConfig.userDataDir, 'home')
...
export function configureOrcaUserDataPathEnv(): void {
  // Why: 重启可能继承旧 CLI 的路径，主进程必须在共享模块构建运行时目录前统一为当前路径。
...
export function migrateLegacyProductUserData(isDev: boolean): void {
  ...
  if (result.status === 'failed') {
    // Why: 继续启动会在新目录写入空状态，导致之后无法安全重试迁移。

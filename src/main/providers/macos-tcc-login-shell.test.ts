...
describe('wrapShellSpawnForMacosTccAttribution', () => {
  ...
  let origPrimaryDisable: string | undefined
  ...
  beforeEach(() => {
    ...
    origPrimaryDisable = process.env.SBBGT_DISABLE_MACOS_LOGIN_SHELL
    origDisable = process.env.ORCA_DISABLE_MACOS_LOGIN_SHELL
    delete process.env.SBBGT_DISABLE_MACOS_LOGIN_SHELL
    ...
    execFileMock.mockImplementation(
      (_file: string, _args: string[], _options: unknown, callback: ExecFileCallback) => {
        callback(null, 'SBBGT_LOGIN_PREFLIGHT_OK', '')
  ...
  afterEach(() => {
    ...
    if (origPrimaryDisable === undefined) {
      delete process.env.SBBGT_DISABLE_MACOS_LOGIN_SHELL
    } else {
      process.env.SBBGT_DISABLE_MACOS_LOGIN_SHELL = origPrimaryDisable
    }
    vi.clearAllMocks()
  })
  ...
  it('wraps the shell in /usr/bin/login on macOS, preserving the shell args behind it', async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(
      '/usr/bin/login',
      ['-flpq', 'ada', '/usr/bin/printf', 'SBBGT_LOGIN_PREFLIGHT_OK'],
      {
        cwd: '/Users/ada',
        encoding: 'utf8',
        killSignal: 'SIGKILL',
  ...
  it('falls back to the plain spawn when disabled via the primary env', () => {
    setPlatform('darwin')
    process.env.SBBGT_DISABLE_MACOS_LOGIN_SHELL = '1'
    expect(wrapShellSpawnForMacosTccAttribution('/bin/zsh', ['-l'])).toEqual({
      file: '/bin/zsh',
      args: ['-l']
    })
  ...
  it('keeps the legacy disable env as a read-only compatibility fallback', () => {
    setPlatform('darwin')

...
description: >
  Control an Android emulator / device from inside 赛博包工头 using the `sbbgt` CLI.
...
# 赛博包工头 Emulator — Android (adb / emulator powered)

Drive an Android emulator or adb-connected device **from within 赛博包工头** using
`SBBGT emulator ...` commands. The Android backend shells out to the Android SDK
...
## Prerequisites (surfaced by 赛博包工头)

- **Android Studio / Android SDK** installed, with `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set. 赛博包工头 also checks the per-OS default location
...
赛博包工头 returns a clear message when the SDK is missing
...
```text
┌────────────────────────┐
│ sbbgt CLI (agents)      │  e.g. SBBGT emulator tap 0.5 0.7 --device emulator-5554
└───────────┬────────────┘
            │ RPC
            ▼
...
赛博包工头 owns backend routing and the per-worktree active-device registry. The
Android backend converts 赛博包工头's normalized 0–1 coordinates to device pixels and
...
- **All coordinates are normalized 0..1** (top-left origin), never pixels — 赛博包工头
  scales to the device's live resolution.
- **Target a running device by its adb serial** (e.g. `emulator-5554`) shown in
  `SBBGT emulator devices`. An AVD name resolves only once that AVD is booted.
...
- Explicit device: `--device <serial>` (recommended for Android today) or an AVD
  name once booted.
- `SBBGT emulator devices` is global (lists every backend's devices); other verbs
...
```text
SBBGT emulator devices --json
SBBGT emulator tap 0.5 0.85 --device emulator-5554 --json
SBBGT emulator type "hello world" --device emulator-5554 --json
SBBGT emulator button recents --device emulator-5554 --json
SBBGT emulator install ./app-debug.apk --reinstall --device emulator-5554 --json
SBBGT emulator launch com.acme.app --device emulator-5554 --json
SBBGT emulator permissions grant com.acme.app android.permission.CAMERA --device emulator-5554 --json
SBBGT emulator ax --device emulator-5554 --json
SBBGT emulator logcat --lines 100 --device emulator-5554 --json
```

## Next action

Run `SBBGT emulator devices --json` to find a booted device, then drive it with

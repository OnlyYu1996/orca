...
name: orchestration
description: >-
  Use 赛博包工头 orchestration for structured multi-agent coordination: threaded
  ...
  ordinary terminal control, lightweight terminal prompts, shell commands, 赛博包工头
  worktree management, reading or waiting on terminals, and automation of the
  browser embedded inside 赛博包工头. Use Computer Use for browser windows, webviews,
  赛博包工头 app UI, or desktop UI outside 赛博包工头's embedded browser.
---

# 赛博包工头 Inter-Agent Orchestration

Orchestration is 赛博包工头's structured coordination layer for agent messages, task ownership, dispatch state, and worker completion tracking.
...
If a task says to use 赛博包工头 orchestration, the coordinator must create 赛博包工头 runtime state with `sbbgt orchestration task-create` and `sbbgt orchestration dispatch --inject` or `sbbgt orchestration run`.

Do not substitute non-赛博包工头 subagent tools, generic agent-spawn APIs, or chat-only parallel worker features. Those may create useful workers, but they do not create 赛博包工头 task/dispatch provenance, injected lifecycle preambles, `worker_done` authority, or decision gates.

Before claiming a worker was orchestrated, verify the task/dispatch exists:

```bash
sbbgt orchestration task-list --json
sbbgt orchestration dispatch-show --task <task_id> --json
```

If the work was accidentally run outside 赛博包工头 orchestration, say so plainly. To repair provenance, rerun or revalidate the needed work through a fresh 赛博包工头 terminal plus injected dispatch; do not retroactively describe the external worker as orchestrated.
...
- `sbbgt status --json` should show a running runtime.
- `sbbgt` must be on PATH (`sbbgt` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `sbbgt orchestration` commands are RPC calls to the running 赛博包工头 runtime.
...
- Do not use `sbbgt orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `sbbgt orchestration task-create`, `sbbgt orchestration dispatch --inject`, or `sbbgt orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
sbbgt orchestration send --to <handle|@group> --subject <text> [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
sbbgt orchestration check [--terminal <handle>] [--unread|--peek|--all] [--types <type,...>] [--inject] [--wait] [--timeout-ms <n>] [--json]
sbbgt orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
sbbgt orchestration ask --to <handle> --question <text> [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
sbbgt orchestration inbox [--limit <n>] [--json]
```

Rules:

- Omit `--from` unless impersonating another terminal; 赛博包工头 auto-resolves it from the current terminal.
...
- Message **one** live agent handle per worker. Use `startupTerminal.handle` from the create response when present; if it is missing or later returns `terminal_handle_stale`, re-resolve with `sbbgt terminal list --worktree ... --json` and continue with the replacement only.
- `sbbgt orchestration check --unread --inject --json` renders unread mail for the agent terminal that runs it; it does not remotely wake another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,decision_gate --timeout-ms <n>` instead of sleep/poll loops. Reply to `decision_gate` messages with `sbbgt orchestration reply --id <msg_id> --body <answer> --json`, then keep waiting.
...
```bash
sbbgt orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
sbbgt orchestration task-list [--status <status>] [--ready] [--brief] [--json]
sbbgt orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
sbbgt orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
sbbgt orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `sbbgt terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
sbbgt orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--json]
sbbgt orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
sbbgt orchestration gate-list [--task <task_id>] [--status <status>] [--json]
sbbgt orchestration run --spec <text> [--from <handle>] [--poll-interval-ms <n>] [--max-concurrent <n>] [--worktree <selector>] [--json]
sbbgt orchestration run-stop [--json]
```
...
Recovery only: `sbbgt orchestration reset --tasks|--messages|--all --json` clears runtime-global orchestration state. Do not run it during active coordination unless explicitly abandoning that state.
...
Before creating a new worktree from an active feature branch, decide and state whether the desired 赛博包工头 lineage is child or top-level. Use child worktree lineage only when the new work is conceptually stacked under or dependent on the active worktree. For independent repo-wide fixes, standalone feature work, or unrelated follow-up tasks, create a top-level worktree with `--no-parent`.
...
`sbbgt worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `sbbgt worktree create --json`; a bare repo id cannot target the new worktree.

```bash
sbbgt worktree create --name <task-name> --no-parent --json
sbbgt terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
...
`--no-parent` only controls 赛博包工头 lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so 赛博包工头 uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `sbbgt repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
For new-worktree workers, read the id and `startupTerminal.handle` from `worktree create`. Use that as the sole worker handle when present; otherwise use `terminal list` to resolve the agent handle. Omit `--repo` only inside an 赛博包工头-managed worktree; otherwise pass `--repo <selector>`.
...
Use `sbbgt worktree create --prompt ...` or `sbbgt terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.

Sidebar lineage and orchestration lifecycle are related but not identical. A same-worktree worker created with `sbbgt terminal create --worktree active` may appear as a peer terminal/agent under the same worktree in the sidebar even though it is a child dispatch in 赛博包工头 orchestration state. A visible parent/child worktree relationship requires creating a child worktree, but do that only when the task can safely run from an isolated checkout and does not need uncommitted artifacts from the current working tree.

Other terminal commands coordinators often need:

```bash
sbbgt terminal list [--worktree <selector>] [--json]
sbbgt terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
sbbgt terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
sbbgt terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
sbbgt terminal read --terminal <handle> --json
sbbgt terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `sbbgt terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal, even on failure:
  `sbbgt orchestration send --to <coordinator_handle> --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","filesModified":["path/a"],"reportPath":"<optional>"}' --json`
- After sending `worker_done`, end your turn and idle at the agent prompt. Do not poll or keep calling `sbbgt orchestration check`; the coordinator re-engages you with a fresh preamble + TASK block delivered as new terminal input.
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `sbbgt orchestration send --to <coordinator_handle> --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
sbbgt terminal create --worktree active --title login-css-worker --command "claude" --json
sbbgt terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
sbbgt orchestration task-create --spec "Fix the login button CSS" --json
sbbgt orchestration dispatch --task <task_id> --to <handle> --inject --json
sbbgt orchestration check --wait --types worker_done,escalation,decision_gate --timeout-ms 900000 --json
```

## Next Action

Coordinator: confirm `sbbgt status --json`, inspect `task-list`/`dispatch-show` if inheriting state, then choose either a manual loop (`task-create` -> worker -> `dispatch --inject` -> `check --wait`) or `orchestration run`.

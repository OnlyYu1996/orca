...
name: computer-use
description: >-
  Use 赛博包工头's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, 赛博包工头 app UI,
  or other desktop UI. Triggers include "computer use", "sbbgt computer", "read
...
Use this skill for desktop UI through `sbbgt computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.
...
- Choose the 赛博包工头 executable once: use the `SBBGT_CLI_COMMAND` environment value when set;
  ...
  everywhere else.
- In every command example, `SBBGT` is a documentation placeholder — including examples that
  name a specific shell. Replace it with that chosen executable before running the command;
  do not create a shell variable or run `SBBGT` literally. Blocks that name no shell are
...
```text
SBBGT status --json
SBBGT computer capabilities --json
```

## Core Loop

```text
SBBGT computer list-apps --json
SBBGT computer get-app-state --app com.spotify.client --json
SBBGT computer click --app com.spotify.client --element-index 42 --json
```
...
```text
SBBGT computer get-app-state --app com.microsoft.edgemac --json
SBBGT computer get-app-state --app Spotify --json
SBBGT computer get-app-state --app pid:12345 --json
```
...
```text
SBBGT computer permissions --json
SBBGT computer capabilities --json
SBBGT computer list-apps --json
SBBGT computer list-windows --app <app> --json
SBBGT computer get-app-state --app <app> --json
SBBGT computer get-app-state --app <app> --restore-window --json
SBBGT computer click --app <app> --element-index <index> --json
SBBGT computer click --app <app> --x 100 --y 100 --json
SBBGT computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
SBBGT computer set-value --app <app> --element-index <index> --value "text" --json
SBBGT computer type-text --app <app> --text "text" --json
SBBGT computer press-key --app <app> --key Return --json
SBBGT computer hotkey --app <app> --key CmdOrCtrl+A --json
SBBGT computer paste-text --app <app> --text "text" --json
SBBGT computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
SBBGT computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
SBBGT computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | SBBGT computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
SBBGT computer get-app-state --app com.microsoft.edgemac --restore-window --json
SBBGT computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
SBBGT computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `SBBGT computer ... --app Gmail` unchanged because `sbbgt computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `SBBGT computer permissions --id screenshots --json`.
- `accessibility_error`: run `SBBGT computer capabilities --json`; if the message names Accessibility permission, run `SBBGT computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `SBBGT computer permissions --json`, or `SBBGT computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm 赛博包工头 status unless already checked, then run `SBBGT computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `SBBGT computer get-app-state --app <app> --json`.

...
description: >-
  Set up, review, debug, or validate 赛博包工头 per-workspace environment recipes —
  ...
  per-workspace environments, fix an `environmentRecipes` entry in `sbbgt.yaml`, scaffold
  provider lifecycle scripts, or resolve an `sbbgt vm recipe doctor` failure.
---
...
赛博包工头 is a **thin wrapper**: you guide, detect, and scaffold; you never own the user's cloud account,
...
**The one branch that shapes everything — connection mode:** **赛博包工头-server** (`create` runs `sbbgt serve`
in the env and emits a `pairingCode`; §7c/§7f) vs **SSH** (`create` runs no server and emits a
`connection.type:"ssh"` block 赛博包工头 dials into; §7g/§7h). Settle this first — it changes the `create`
output shape and half the templates.

**Quick-start (happy path):** interview the user (connection mode 赛博包工头-server vs SSH, provider, agent CLI,
git auth — §1.2) + read the provider's CLI docs → scaffold `scripts/orca-vm/` from §7 → run the
base-snapshot script, then the auth script (you invoke these by hand; not via `sbbgt.yaml`) → wire
`environmentRecipes` in `sbbgt.yaml` → `sbbgt vm recipe doctor <id> --json` (free) → then the `--provision`
...
a long time, or need the user at the keyboard. Never create an 赛博包工头 workspace or commit unless asked.
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how 赛博包工头 attaches to the environment — an **赛博包工头 server** (the VM runs
     `sbbgt serve` and 赛博包工头 pairs over its pairing URL; worked example §7f) or **SSH** (赛博包工头 connects to
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, … For non-obvious providers, also
     ...
     (host/port/user/key or proxy command) or only a provider-mediated interactive shell; 赛博包工头 SSH mode
     needs the former.
...
7. **Wire the recipe** so `sbbgt.yaml` points create/suspend/resume/destroy at the scripts (§8). The
   workspace composer reads `environmentRecipes` from the project's primary checkout of `sbbgt.yaml`, **not** from
   a feature branch or worktree. So a recipe added only on a branch won't appear as a "Run on" option
   until that `sbbgt.yaml` change is committed and merged to the project's primary branch. Tell the user
...
8. **Dry-run doctor** — `sbbgt vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `sbbgt vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Connection mode** (赛博包工头 server vs SSH) confirmed with the user — see §1 step 2; it shapes the recipe.
...
- **Local-side** (`create`/`suspend`/`resume`/`destroy` + the base-snapshot/auth scripts the user
  ...
  or require WSL/Git-Bash and point `sbbgt.yaml` at the right launcher.
...
Worked Vercel commands for this phase are in §7f. You run this script by hand (not via `sbbgt.yaml`),
...
# name = orca-${SBBGT_VM_RECIPE_ID}-${SBBGT_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. remote exec: start sbbgt serve in the background and read the recipe JSON it writes (see below)
...
**The exact `sbbgt serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec sbbgt-dev serve …`
from the repo root — `sbbgt-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`sbbgt serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
```json
{ "schemaVersion": 1, "pairingCode": "<sbbgt pairing URL>", "projectRoot": "<the --project-root you passed>" }
```
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # 赛博包工头 passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in sbbgt.yaml)
```
...
name="orca-${SBBGT_VM_RECIPE_ID:-vercel-sandbox}-${SBBGT_VM_INSTANCE_ID:-$(date +%s)}"  # sanitize+cap to 63 chars
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "SBBGT_PROJECT_ROOT=$project_root" \
  --env "SBBGT_REPO_URL=$repo_url" --env "SBBGT_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$SBBGT_PROJECT_ROOT"; \
    ...
    git fetch origin "$SBBGT_REPO_REF"; \
    git checkout -B "$SBBGT_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
...
# 3. (remote) start sbbgt serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "SBBGT_PORT=$port" --env "SBBGT_PROJECT_ROOT=$project_root" --env "SBBGT_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$SBBGT_PROJECT_ROOT"; rm -f /tmp/orca-recipe.json /tmp/orca-serve.log; \
    nohup pnpm exec sbbgt-dev serve --port "$SBBGT_PORT" --project-root "$SBBGT_PROJECT_ROOT" \
      --pairing-address "$SBBGT_PAIRING_ADDRESS" --recipe-json >/tmp/orca-recipe.json 2>/tmp/orca-serve.log </dev/null & \
...
`userData.resourceId` from stdin (§7d). This is the **赛博包工头-server** connection mode (the recipe emits a
...
- **`create` does NOT run `sbbgt serve` and does NOT emit a `pairingCode`.** 赛博包工头 itself connects to the
  host over its SSH relay, brings up the git + filesystem providers, and imports the repo. The script's
  only job is to make the host ready and **print SSH connection details** 赛博包工头 will dial.
- The result uses a `connection` block with `type: "ssh"` and a `target`, **not** the flat
  `pairingCode`/`projectRoot` shape. Exact shape (赛博包工头 rejects anything else):
...
- `relayGracePeriodSeconds` (optional): how long 赛博包工头 keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO sbbgt serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO sbbgt serve). host/port/username tell 赛博包工头's
...
`destroy: none` and omit suspend/resume. (赛博包工头 still disconnects/reconnects its own SSH relay on
...
`connection.type:"ssh"` block above instead of starting `sbbgt serve`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "SBBGT_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
require WSL/Git-Bash (and point `sbbgt.yaml` at e.g. `bash ./scripts/orca-vm/<name>.sh` via a `.cmd`
...
# 赛博包工头-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
...
**赛博包工头-server mode** — boot the env, start `sbbgt serve` in it, and print serve's result:
...
Here `pairingCode` (from `sbbgt serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `sbbgt serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start 赛博包工头 remotely with `sbbgt serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`sbbgt vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`sbbgt vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
...
- Don't make 赛博包工头 own provider lifecycle beyond invoking the configured scripts.
- Don't commit or create an 赛博包工头 workspace unless asked.
